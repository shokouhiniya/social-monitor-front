...
export function SentimentTimeline({ data }) {
  ...
  const chartData = items
    .filter(item => Number(item.post_count) > 0)
    .map((item) => {
      ...
      <Box sx={{ height: 200, direction: 'ltr' }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="jalaliDate" tick={{ fontSize: 9, fill: theme.palette.text.secondary }} interval="preserveStartEnd" />
            ...
            <Area type="monotone" dataKey="avg_sentiment" stroke={theme.palette.primary.main} fill="url(#sentGrad)" strokeWidth={2}
              dot={(props) => {
                ...
                return (
                  <circle cx={cx} cy={cy} r={size} fill={theme.palette.primary.main}
                    stroke={theme.palette.background.paper} strokeWidth={1.5} opacity={0.8} />
                );
              }}
            />
          </AreaChart>
