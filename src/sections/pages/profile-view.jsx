...
export function PageProfileView({ id }) {
  ...
  if (isLoading) {
    return (<DashboardContent maxWidth="xl"><Box sx={{ py: 10, textAlign: 'center' }}><CircularProgress /></Box></DashboardContent>);
  }

  if (!data) {
    return (<DashboardContent maxWidth="xl"><Typography>پیج یافت نشد</Typography></DashboardContent>);
  }
  ...
  const handleEditOpen = () => {
    setEditForm({
      name: pg.name || '', username: pg.username || '', platform: pg.platform || 'instagram',
      category: pg.category || '', country: pg.country || '', language: pg.language || '', bio: pg.bio || '',
    });
    setEditOpen(true);
  };

  const handleEditSave = () => {
    updateMutation.mutate({ id, data: editForm }, { onSuccess: () => setEditOpen(false) });
  };

  const handleRefine = (field) => {
  ...
  const handleRefineSubmit = () => {
    // TODO: Send to LLM with user note for refinement
    setRefineOpen(false);
  };
  ...
  return (
    <DashboardContent maxWidth="xl">
      <Grid container spacing={3}>
        ...
        {[
          { title: 'اعتبار', value: data.credibility_score, prev: data.credibility_score * 0.9, icon: 'solar:shield-check-bold-duotone', color: data.credibility_score > 7 ? 'success' : data.credibility_score > 4 ? 'warning' : 'error', info: 'میزان جدی گرفته شدن توسط شبکه', field: 'credibility_score' },
          { title: 'نفوذ', value: data.influence_score, prev: data.influence_score * 0.85, icon: 'solar:crown-bold-duotone', color: 'primary', info: 'ضریب تاثیرگذاری بر سایر پیج‌ها', field: 'influence_score' },
          { title: 'پایداری', value: data.consistency_rate, prev: data.consistency_rate * 0.95, icon: 'solar:clock-circle-bold-duotone', color: data.consistency_rate > 7 ? 'success' : 'warning', info: 'مداومت در انتشار محتوا', field: 'consistency_rate' },
        ...
        {/* Psychology */}
        <Grid size={{ xs: 12, md: 5 }}><PersonaRadar data={data.persona_radar} /></Grid>
        <Grid size={{ xs: 12, md: 7 }}><SentimentTimeline data={data.sentiment_timeline} /></Grid>

        {/* Network */}
        <Grid size={{ xs: 12, md: 6 }}><NetworkCircle page={pg} relatedPages={relatedPages} /></Grid>
        <Grid size={{ xs: 12, md: 6 }}>
          <NarrativeTimeline posts={pg?.posts} fieldReports={pg?.field_reports} contentHooks={data.content_hooks} />
        </Grid>
      </Grid>
      ...
      <Dialog open={refineOpen} onClose={() => setRefineOpen(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: 2 } }}>
        <DialogTitle>بهبود شاخص «{refineField}»</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            توضیحات خود را بنویسید تا سیستم با کمک هوش مصنوعی شاخص را اصلاح کند.
          </Typography>
        ...
        <DialogActions>
          ...
          <Button variant="contained" color="warning" onClick={handleRefineSubmit} disabled={!refineNote}
